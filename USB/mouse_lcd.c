...
{ 
	static uint8_t b_state[5] = { 0, 0 , 0 , 0 , 0};
...
void HID_MOUSE_UpdatePosition (int8_t x, int8_t y)
{
  ...
  {	    
    ...
    if((x_loc > 150) && (y_loc < 100)) {

    	PELCO_D_command_rotation_up_right(1,((x_loc-150)/10) , (((y_loc-100)*(-1))/8));
    }

    if((x_loc > 150) && (y_loc > 100)) {

    	PELCO_D_command_rotation_down_right(1,((x_loc-150)/10) ,((y_loc-100)/8) );

    }

    if((x_loc < 150) && (y_loc < 100)) {

    	PELCO_D_command_rotation_up_left(1 , (((x_loc-150)*(-1))/10) ,(((y_loc-100)*(-1))/8) );
    }

    if((x_loc < 150) && (y_loc > 100)) {

    	PELCO_D_command_rotation_down_left(1 , (((x_loc-150)*(-1))/10) ,((y_loc-100)/8) );
...
void HID_MOUSE_ButtonPressed(uint8_t button_idx)
{
  ...
  case 3 :
    LCD_DrawFullRect(HID_MOUSE_BUTTON_XCHORD,HID_MOUSE_BUTTON4_YCHORD,\
      HID_MOUSE_BUTTON_WIDTH,HID_MOUSE_BUTTON_HEIGHT);
    break;

  case 4 :
     LCD_DrawFullRect(HID_MOUSE_BUTTON_XCHORD,HID_MOUSE_BUTTON5_YCHORD,\
       HID_MOUSE_BUTTON_WIDTH,HID_MOUSE_BUTTON_HEIGHT);
     break;
