...
int sl811Initformouse(void)
{
	...
	sl811_writereg(EP0Control,sDATA0_RD);   	/* start generate SOF or EOP */ //sDATA0_RD
...
int MouseInit(void)
{
	...
	if( ( sl811Initformouse() == 0) && (EnumUsbDevformouse(2) == 1))
	{
	...
	while(1)
	{
		if(!Slave_Detach())		//!Slave_Detach()
		{
			...
			if( DATA_INPROCESS )
			{
				...
				if( DBUF[0] & 0x03 )
				{
					ReceiveMouseData.button_4 = 1;
				}
				else
				{
					ReceiveMouseData.button_4 = 0;
				}
				if( DBUF[0] & 0x05 )
				{
					ReceiveMouseData.button_5 = 1;
				}
				else
				{
					ReceiveMouseData.button_5 = 0;
				}
